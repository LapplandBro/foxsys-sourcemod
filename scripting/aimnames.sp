...
public OnPluginStart()
{
	...
	g_Cvar_Color = CreateConVar("sm_aimnames_color", "1", "Default color for hudtext. (0 = red, 1 = orange, 2 = yellow, 3 = green, 4 = blue)");
	...
	g_Cvar_Health = CreateConVar("sm_aimnames_health", "0", "Enable/Disable admins viewing players health.");
...
public Menu_CookieSettingsEnable(Handle:menu, MenuAction:action, param1, param2)
{
	...
	if (action == MenuAction_Select) 
	{
		...
		if (StrEqual(sSelection, "enable", false))
		{
			...
			MC_PrintToChat(client, "{GREEN}[{LIGHTGREEN}Aim Naming{GREEN}]{DEFAULT} При наведении на противника {LIGHTGREEN}будет{DEFAULT} показываться его никнейм.");
		...
		else
		{
			...
			g_aPlayers[client][bHudEnabled] = false;
			MC_PrintToChat(client, "{GREEN}[{LIGHTGREEN}Aim Naming{GREEN}]{DEFAULT} При наведении на противника {LIGHTGREEN}не будет{DEFAULT} показываться его никнейм.");
			if (g_aPlayers[client][hHudTimer] != INVALID_HANDLE)
			{
				ClearTimer(g_aPlayers[client][hHudTimer]);
			}
...
public Menu_CookieHealthEnable(Handle:menu, MenuAction:action, param1, param2)
{
	...
	if (action == MenuAction_Select) 
	{
		...
		if (StrEqual(sSelection, "henable", false))
		{
			...
			MC_PrintToChat(client, "{GREEN}[{LIGHTGREEN}Aim Naming{GREEN}]{DEFAULT} Отображение здоровья включено.");
		}
		else
		{
			SetClientCookie(client, g_hCookie_ShowHealth, "1");
			g_aPlayers[client][bHealthEnabled] = false;
			MC_PrintToChat(client, "{GREEN}[{LIGHTGREEN}Aim Naming{GREEN}]{DEFAULT} Отображение здоровья отключено.");
		}
	}
	else if (action == MenuAction_Cancel) 
	{
...
stock SendCookieSettingsMenu(client)
{
	...
	SetMenuTitle(hMenu, "Опции (Текущие)");
	if (g_aPlayers[client][bHudEnabled])
	{
		AddMenuItem(hMenu, "enable", "Вкл/выкл (Вкл)");
	}
	else
	{
		AddMenuItem(hMenu, "enable", "Вкл/выкл (Выкл)");
	}
	if (g_aPlayers[client][bIsAdmin])
	{
		if (g_aPlayers[client][bHealthEnabled])
		{
			AddMenuItem(hMenu, "health", "Показ здоровья ВКЛ");
		}
		else
		{
			AddMenuItem(hMenu, "health", "Показ здоровья ВЫКЛ");
		}
	}
	switch (g_aPlayers[client][eColor])
	{
		case Color_Red:
		{
			AddMenuItem(hMenu, "color", "Цвет (Красный)");
		}
		case Color_Orange:
		{
			AddMenuItem(hMenu, "color", "Цвет (Оранжевый)");
		}
		case Color_Yellow:
		{
			AddMenuItem(hMenu, "color", "Цвет (Желтый)");
		}
		case Color_Green:
		{
			AddMenuItem(hMenu, "color", "Цвет (Зеленый)");
		}
		case Color_Blue:
		{
			AddMenuItem(hMenu, "color", "Цвет (Синий)");
		}
	}
...
stock SendCookieEnabledMenu(client)
{
	...
	SetMenuTitle(hMenu, "Вкл/Выкл показ ника:");
	
	if (g_aPlayers[client][bHudEnabled])
	{
		AddMenuItem(hMenu, "enable", "Вкл. (Текущий)");
		AddMenuItem(hMenu, "disable", "Выкл.");
	}
	else
	{
		AddMenuItem(hMenu, "enable", "Вкл.");
		AddMenuItem(hMenu, "disable", "Выкл. (Текущий)");
	}
...
stock SendCookieColorMenu(client)
{
	...
	SetMenuTitle(hMenu, "Выберите цвет текста в худе:");
	switch (g_aPlayers[client][eColor])
	{
		case Color_Red:
		{
			AddMenuItem(hMenu, "Red", "Красный (Уст.)");
			AddMenuItem(hMenu, "Orange", "Оранжевый");
			AddMenuItem(hMenu, "Yellow", "Желтый");
			AddMenuItem(hMenu, "Green", "Зеленый");
			AddMenuItem(hMenu, "Blue", "Синий");
		}
		case Color_Orange:
		{
			AddMenuItem(hMenu, "Red", "Красный");
			AddMenuItem(hMenu, "Orange", "Оранжевый (Уст.)");
			AddMenuItem(hMenu, "Yellow", "Желтый");
			AddMenuItem(hMenu, "Green", "Зеленый");
			AddMenuItem(hMenu, "Blue", "Синий");
		}
		case Color_Yellow:
		{
			AddMenuItem(hMenu, "Red", "Красный");
			AddMenuItem(hMenu, "Orange", "Оранжевый");
			AddMenuItem(hMenu, "Yellow", "Желтый (Уст.)");
			AddMenuItem(hMenu, "Green", "Зеленый");
			AddMenuItem(hMenu, "Blue", "Синий");
		}
		case Color_Green:
		{
			...
			AddMenuItem(hMenu, "Orange", "Оранжевый");
			AddMenuItem(hMenu, "Yellow", "Желтый");
			AddMenuItem(hMenu, "Green", "Зеленый (Уст.)");
			AddMenuItem(hMenu, "Blue", "Синий");
		}
		case Color_Blue:
		{
			...
			AddMenuItem(hMenu, "Yellow", "Желтый");
			AddMenuItem(hMenu, "Green", "Зеленый");
			AddMenuItem(hMenu, "Blue", "Синий (Уст.)");
		}
	}
	SetMenuExitBackButton(hMenu, true);
	DisplayMenu(hMenu, client, MENU_TIME_FOREVER);
...
stock SendCookieHealthMenu(client)
{
	...
	SetMenuTitle(hMenu, "Вкл/Выкл показ HP");
	
	if (g_aPlayers[client][bHealthEnabled])
	{
		AddMenuItem(hMenu, "henable", "Вкл. (Уст.)");
		AddMenuItem(hMenu, "hdisable", "Выкл.");
	}
	else
	{
		AddMenuItem(hMenu, "henable", "Вкл.");
		AddMenuItem(hMenu, "hdisable", "Выкл. (Уст.)");
	}
