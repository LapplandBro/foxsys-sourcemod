...
public Plugin myinfo = 
{
	...
	author = "Pelipoika, Lappland Saluzzo",
	description = "",
	version = "1.0.1",
...
public void OnPluginStart() {
	RegConsoleCmd("sm_rainbowme", Command_Rainbow);
...
public void OnPluginEnd() {
	MC_PrintToChatAll("{GREEN}[{LIGHTGREEN}Rainbow Me{GREEN}]{DEFAULT} Подсветка всех моделей выключена, плагин выгружен!");
	int index = -1;
	...
	if(client > 0 && client <= MaxClients && IsClientInGame(client))
	{
		...
		{	
			...
			if(IsValidEntity(iGlow))
			{
				...
				MC_PrintToChat(client, "{GREEN}[{LIGHTGREEN}Rainbow Me{GREEN}]{DEFAULT} Подсветка вашей модели - включена.");
			}
		}
		else
		{
			...
			if(iGlow != INVALID_ENT_REFERENCE)
			{
				...
				MC_PrintToChat(client, "{GREEN}[{LIGHTGREEN}Rainbow Me{GREEN}]{DEFAULT} Подсветка вашей модели - выключена.");
			}
		}
	}
