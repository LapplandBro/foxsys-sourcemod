#define PLUGIN_NAME "[TF2] Sheens Kit"
#define PLUGIN_AUTHOR "Lappland Saluzzo"
	...
	public void Show(int client, char type, char[] data = "") {
		...
		switch (type) {
			case KS_SHEEN, KS_BOTH: {
				this.SetTitle("Выберите сияние KillStreak:");
				this.AddItemArray(sSheens, sizeof(sSheens));
				this.ExitBackButton = true;
			}
			case KS_EFFECT: {
				this.SetTitle("Выберите эффект KillStreak:");
				this.AddItemArray(sEffects, sizeof(sEffects));
				this.ExitBackButton = true;
			}
			default: {
				this.SetTitle("Выберите тип Killstreak:");
				this.AddItem("1", "Выключить");
				this.AddItem("2", "Нормальный");
				this.AddItem("3", "Особо опасный");
				this.AddItem("4", "Профессиональный");
			}
		}
	...
	if (!IsPlayerAlive(client))ReplyToCommand(client, "%s %t", PLUGIN_NAME, "player must be alive");
		...
		if (!IsValidEntity(weapon))ReplyToCommand(client, "[%s] %t", PLUGIN_NAME, "invalid weapon");
			...
			if (sheen == -2) {
				...
				MC_ReplyToCommand(client, "{GREEN}%s{DEFAULT} %t", PLUGIN_NAME, "disabled killstreak");
			}
			else if (sheen == -1) {
				Attributes.Remove(weapon, 2013);
				Attributes.Remove(weapon, 2014);
				Attributes.Set(weapon, 2025, 1.0);
				MC_ReplyToCommand(client, "{GREEN}%s{DEFAULT} %t", PLUGIN_NAME, "enabled killstreak");
			}
			else if (effect >= 0) {
				Attributes.Set(weapon, 2013, float(effect + 2002));
				Attributes.Set(weapon, 2014, float(sheen + 1));
				Attributes.Set(weapon, 2025, 3.0);
				MC_ReplyToCommand(client, "{GREEN}%s{DEFAULT} %t", PLUGIN_NAME, "set sheen to x and effect to y", sSheens[sheen], sEffects[effect]);
			}
			else {
				Attributes.Remove(weapon, 2013);
				Attributes.Set(weapon, 2014, float(sheen + 1));
				Attributes.Set(weapon, 2025, 2.0);
				MC_ReplyToCommand(client, "{GREEN}%s{DEFAULT} %t", PLUGIN_NAME, "set sheen to x", sSheens[sheen]);
			}
		}
	}
}
