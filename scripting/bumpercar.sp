...
public Plugin:myinfo = {
    name = "[TF2] Bumper Car",
    author = "Chdata, Lappland Saluzzo",
...
public Action:cmdBumperCar(client, argc)
{
    if (argc == 0)
    {
        if (!client)
        {
            ReplyToCommand(client, "[Bumper Car] Console cannot ride in bumper cars.");
            return Plugin_Handled;
        }

        if (!CheckCommandAccess(client, "adm_bumpercar_self", 0))
        {
            ReplyToCommand(client, "[Bumper Car] %t.", "No Access");
            return Plugin_Handled;
        }
    ...
    else
    {
        ...
        {
            MC_ReplyToCommand(client, "{GREEN}[{LIGHTGREEN}Bumper Cars{GREEN}]{DEFAULT} У вас нет прав выбора целей.");
            return Plugin_Handled;
        }
                ...
                tn_is_ml)) > 0)
        {
            ...
            {
                MC_ReplyToCommand(client, "{GREEN}[{LIGHTGREEN}Bumper Cars{GREEN}]{DEFAULT} Использование: !car - включает или выключает машинку;");
                return Plugin_Handled;
            }
            else
            {
            ...
            MC_ShowActivity2(client, "{GREEN}[{LIGHTGREEN}Bumper Cars{GREEN}]{DEFAULT} ", "%s бамперная машинка на игроке %s.", !bOn?"Убрана":"Установлена", target_name);
...
stock bool:SelfEnterCar(iClient) // , iOn=-1
{
    ...
    {
        MC_ReplyToCommand(iClient, "{GREEN}[{LIGHTGREEN}Bumper Cars{GREEN}]{DEFAULT} Только команда %s может ездить на машинках.", g_iCatTeam == 2 ? "{RED}красных{DEFAULT}" : "{BLUE}синих{DEFAULT}");
        return false;
    }
    ...
    if (!IsPlayerAlive(iClient))
    {
        ...
        if (iKeepCar == 1)
        {
            MC_ReplyToCommand(iClient, "{GREEN}[{LIGHTGREEN}Bumper Cars{GREEN}]{DEFAULT} Вы будете в машинке при возрождении!");
            return true;
        }
        else if (iKeepCar == 0)
        {
            MC_ReplyToCommand(iClient, "{GREEN}[{LIGHTGREEN}Bumper Cars{GREEN}]{DEFAULT} Вы должны быть живы для бамперной машинки!");
            return false;
        }
    }
    ...
    {
        MC_ReplyToCommand(iClient, "{GREEN}[{LIGHTGREEN}Bumper Cars{GREEN}]{DEFAULT} У вас будет %s машинка при возрождении!", bIsInKart ? "отключена" : "включена");
        return !bIsInKart;
    }
    if (bIsInKart)
    {
        ...
        MC_ReplyToCommand(iClient, "{GREEN}[{LIGHTGREEN}Bumper Cars{GREEN}]{DEFAULT} Вы вышли из машинки!");
        return false;
    }
    if (!TryEnterCar(iClient))
    {
        MC_ReplyToCommand(iClient, "{GREEN}[{LIGHTGREEN}Bumper Cars{GREEN}]{DEFAULT} В вашем состоянии невозможно усадить вас в машинку!");
        return false;
    }
    MC_ReplyToCommand(iClient, "{GREEN}[{LIGHTGREEN}Bumper Cars{GREEN}]{DEFAULT} Вы вошли в машинку!");
    return true;
}
