...
{
	name = "[TF2] Be the Skeleton",
	author = "Mitchell, Lappland Saluzzo",
...
//	AddNormalSoundHook(SoundHook);
	...
	CreateConVar("sm_betheskeleton_version",PLUGIN_VERSION,"Plugin version.", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_SPONLY);
...
public Action:Command_betheskeleton(client, args)
{
	if (!client && !args)
	{
		...
		MC_ReplyToCommand(client, "{GREEN}[{LIGHTGREEN}Be The Skeleton{GREEN}]{DEFAULT} Использование: %s <name|#userid> [1/0] - Переключает в скелета.", arg0);
		return Plugin_Handled;
	}
	if (!CheckCommandAccess(client, "betheskeleton", 0))
	{
		MC_ReplyToCommand(client, "{GREEN}[{LIGHTGREEN}Be The Skeleton{GREEN}]{DEFAULT} %t.", "No Access");
		return Plugin_Handled;
	}
	...
	{
		if (!ToggleSkeleton(client)) MC_ReplyToCommand(client, "{GREEN}[{LIGHTGREEN}Be The Skeleton{GREEN}]{DEFAULT} Для применения изменений - возьмите класс снайпера!");
		return Plugin_Handled;
	}
	else
	{
	...
	if (toggle != false && toggle != true) MC_ShowActivity2(client, "{GREEN}[{LIGHTGREEN}Be The Skeleton{GREEN}]{DEFAULT} ", "Режим робота установлен на игроке - %s.", target_name);
	else MC_ShowActivity2(client, "{GREEN}[{LIGHTGREEN}Be The Skeleton{GREEN}]{DEFAULT} ", "%sключен режим робота на игроке %s.", toggle ? "В" : "Вы", target_name);
	return Plugin_Handled;
}
