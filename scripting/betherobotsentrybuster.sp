...
{
	name = "[TF2] BTR Buster",
	author = "MasterOfTheXP, Lappland Saluzzo",
...
public Action:Command_bethebuster(client, args)
{
	if (!client && !args)
	{
		...
		ReplyToCommand(client, "[Be The Buster] Using: %s <name|#userid> [1/0]", arg0);
		return Plugin_Handled;
	}
	if (!CheckCommandAccess(client, "bethebuster", ADMFLAG_ROOT))
	{
		ReplyToCommand(client, "[Be The Buster] %t.", "No Access");
		return Plugin_Handled;
	}
	...
	if (args < 1 || !CheckCommandAccess(client, "bethebuster_admin", ADMFLAG_ROOT))
	{
		...
		if (!ToggleBuster(client)) MC_ReplyToCommand(client, "{GREEN}[{LIGHTGREEN}Be The Robot{GREEN}]{DEFAULT} Вы станете бастером, как только это станет возможно!");
		return Plugin_Handled;
	}
	else
	{
	...
	if (toggle != false && toggle != true) MC_ShowActivity2(client, "{GREEN}[{LIGHTGREEN}Be The Buster{GREEN}]{DEFAULT} ", "Бастер на игроке %s переключен.", target_name);
	else MC_ShowActivity2(client, "{GREEN}[{LIGHTGREEN}Be The Buster{GREEN}]{DEFAULT} ", "%sключен бастер на игроке %s.", toggle ? "В" : "Вы", target_name);
	return Plugin_Handled;
}

stock bool:ToggleBuster(client, bool:toggle = bool:2)
{
	...
	if (Status[client] == BusterStatus_Buster && toggle == true && BeTheRobot_CheckRules(client)) return true;
	...
	else if (!toggle || (toggle == bool:2 && Status[client] == BusterStatus_Buster))
	{
		SetVariantString("");
		AcceptEntityInput(client, "SetCustomModel");
		Status[client] = BusterStatus_Human;
