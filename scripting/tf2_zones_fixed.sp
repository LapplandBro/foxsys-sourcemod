				...
				switch (real_punishment)
				{
					...
					case SLAY:
					{
						...
						if (StartTouch)
						{
							...
							{
								// Kill player using this native, because sometimes players wont die in DoD:S | Because Timers, this not immediatly kill player if used more in 5 seconds;
								SDKHooks_TakeDamage(activator, 0, 0, float(GetClientHealth(activator)));
							}
								else
							{
								// For other games use ForcePlayerSuicide | This is not working on TF2, if cart activated in plugin BumperCar
								if (CurrentVersion == Engine_TF2) {
									if  (TF2_IsPlayerInCondition(activator, TFCond_HalloweenKart)) {
										TF2_RemoveCondition(activator, TFCond_HalloweenKart);
									}
								}
								SDKHooks_TakeDamage(activator, 0, 0, float(GetClientHealth(activator))); // Because in plugin BumperCars - enabled suicide.
								// FakeClientCommand(activator, "kill"); // Reserved;
