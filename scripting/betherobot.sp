...
public Action:Command_betherobot(client, args)
{
	if (!client && !args)
	{
		...
		ReplyToCommand(client, "[Be The Robot] Using: %s <name|#userid> [1/0]", arg0);
		return Plugin_Handled;
	}
	if (!CheckCommandAccess(client, "betherobot", 0))
	{
		ReplyToCommand(client, "[Be The Robot] %t.", "No Access");
		return Plugin_Handled;
	}
	...
	{
		if (!ToggleRobot(client)) MC_ReplyToCommand(client, "{GREEN}[{LIGHTGREEN}Be The Robot{GREEN}]{DEFAULT} Вы станете роботом, как только это станет возможно!");
		return Plugin_Handled;
	}
	else
	{
	...
	if (toggle != false && toggle != true) MC_ShowActivity2(client, "{GREEN}[{LIGHTGREEN}Be The Robot{GREEN}]{DEFAULT} ", "Режим робота установлен на игроке %s.", target_name);
	else MC_ShowActivity2(client, "{GREEN}[{LIGHTGREEN}Be The Robot{GREEN}]{DEFAULT} ", "%sключен режим робота на игроке %s.", toggle ? "В" : "Вы", target_name);
	return Plugin_Handled;
}
