...
public Action:AP_Command(myClient, args)
{
	...
	if (!AP_ENABLED[myClient])
	{
    	...
    	MC_ReplyToCommand(myClient, "{GREEN}[DPA]{WHITE} Помощник по Demo Pogo включен!");
    	if (TF2_GetPlayerClass(myClient) != TFClass_DemoMan)
    	{
    		...
    		ShowSyncHudText(myClient, HudDisplay, "Класс на подрывника смени!");
    	}
    	else
    	{
     		ClearSyncHud(myClient, HudDisplay);
    		SetHudTextParams(-1.0, -1.0, 3.0, 0, 255, 0, 255, 0, 0.1, 0.1, 0.1);
    		ShowSyncHudText(myClient, HudDisplay, "Помощник включен!");
    	}
    	return Plugin_Continue;
    }
	if (AP_ENABLED[myClient])
    {
    	AP_ENABLED[myClient] = false;
    	MC_ReplyToCommand(myClient, "{GREEN}[DPA]{WHITE} Помощник по Demo Pogo выключен!");
    	Projectile[myClient][0] = -1;
    }
...
public Action:APF_Command(myClient, args)
{
	...
	if (!APF_ENABLED[myClient])
	{
    	...
    	MC_ReplyToCommand(myClient, "{GREEN}[DPA]{WHITE} Помощник Demo Pogo по высоте включен!");
    	if (TF2_GetPlayerClass(myClient) != TFClass_DemoMan)
    	{
    		ClearSyncHud(myClient, HudDisplay);
    		SetHudTextParams(-1.0, -1.0, 3.0, 255, 255, 255, 255, 0, 0.1, 0.1, 0.1);
    	...
    	else
    	{
     		ClearSyncHud(myClient, HudDisplay);
    		SetHudTextParams(-1.0, -1.0, 3.0, 0, 255, 0, 255, 0, 0.1, 0.1, 0.1);
    		ShowSyncHudText(myClient, HudDisplay, "Помощник включен! Показ высоты активирован.");
    	}
    	return Plugin_Continue;
    }
	if (APF_ENABLED[myClient])
    {
    	APF_ENABLED[myClient] = false;
    	MC_ReplyToCommand(myClient, "{GREEN}[DPA]{WHITE} Помощник Demo Pogo по высоте выключен!");
    }
...
public Action:ProjectileSpawn(int myEntity)
{
	...
	if (PL_Enabled && PrevRef != myRef)
	{
		...
		if (AP_ENABLED[myOwner] && TF2_GetPlayerClass(myOwner) == TFClass_DemoMan && IsValidEntity(myEntity) && (WeaponIndex == 20 || WeaponIndex == 207 || WeaponIndex == 661 || WeaponIndex == 797 || WeaponIndex == 806 || WeaponIndex == 886 || WeaponIndex == 895 || WeaponIndex == 904 || WeaponIndex == 913 || WeaponIndex == 962 || WeaponIndex == 971))
		{
...
public Action:OnPlayerRunCmd(myClient, &myButtons, &myImpulse, Float:myVel[3], Float:myAng[3], &myWeapon)
{
	...
	if (AP_ENABLED[myClient] && myClient == GetEntPropEnt(Projectile[myClient][0], Prop_Send, "m_hThrower"))
	{
		...
		ShowSyncHudText(myClient, HudDisplay, "Дистанция до объекта: %1.0f \n\nДистанция взрыва: %s", FL_Dist / 6.0, chart);
...
stock bool:IsValidClient(myClient, bool:Replay = true)
{
  if(myClient <= 0 || myClient > MaxClients || !IsClientInGame(myClient))
    return false;
  if(Replay && (IsClientSourceTV(myClient) || IsClientReplay(myClient) || IsClientObserver(myClient) || IsFakeClient(myClient)))
    return false;
  return true;
}
