...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_ragdoll", Cmd_Ragdoll);
...
public void RagdollMenu(int client)
{
	...
	menu.SetTitle("Выберите эффект трупов:");
	menu.AddItem("0", "Без Регдолла", (ragdollType[client] == 0) ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);
	menu.AddItem("1", "Горение", (ragdollType[client] == 1) ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);
	menu.AddItem("2", "Электрошок", (ragdollType[client] == 2) ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);
	menu.AddItem("3", "Австралий", (ragdollType[client] == 3) ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);
	menu.AddItem("4", "Лед", (ragdollType[client] == 4) ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);
	menu.AddItem("5", "Трамплин", (ragdollType[client] == 5) ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);
	menu.AddItem("6", "Дезинтеграция", (ragdollType[client] == 6) ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);
	menu.AddItem("8", "Случайный", (ragdollType[client] == 8) ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);
...
public int RagdollMenuHandler(Menu menu, MenuAction action, int client, int param2)
{
	...
	if (action == MenuAction_Select)
	{
		...
		MC_PrintToChat(client, "{GREEN}[{LIGHTGREEN}Set Ragdoll{GREEN}]{DEFAULT} Трупы ваших врагов будут иметь эффект: {LIGHTGREEN}%s{DEFAULT}.", display);
		RagdollMenu(client);
	}
}
